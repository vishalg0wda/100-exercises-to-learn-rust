...
impl WeekTemperatures {
    pub fn new() -> Self {
        Self{ readings: [None; 7]}
    }

    fn day_idx(day: Weekday) -> usize {
        match day {
            Weekday::Monday => 0,
            Weekday::Tuesday => 1,
            Weekday::Wednesday => 2,
    ...
    pub fn get_temperature(&self, day: Weekday) -> Option<i32> {
        self.readings[WeekTemperatures::day_idx(day)].map(|temp| temp)
    }

    pub fn set_temperature(&mut self, day: Weekday, temperature: i32) {
        self.readings[WeekTemperatures::day_idx(day)] = Some(temperature);
    }
}
